clc
clearvars -except estructuras results
close all
...
experimento = input('Tipo de experimento (1-Velocidad,2-Angulo,3-Tiempo, 4-StairCase vel 6-StairCase tiempo) ');
...
newFile = input('�Crear un archivo nuevo? 0-no 1-si');
if newFile
%% ANGULOS A PRESENTAR
    switch experimento
        case 1
    ...
    %           posiblesAngulosF =  [-6,6,2,-2,10,-10,3,-3];
        ...
        case 3
            estructuras = createStair(); %creates the str for the staircase
            if mono == 'd'
        ...
        case 4 %staircase speed
            velMin = 0.2;
            velMax = 1.0;
            bins = 1:10;
            estructuras = createStair()
            if mono == 'c'
                xUnit = 1; %what we can consider as a unit of change in the value
                xMin = 0.1;
                xInicial = 4;
                estructuras = createStair(xInicial,xUnit,xMin)
                posiblesAngulosI = [0,-4,4]; %angulos iniciales
                posiblesAngulosF = [0.5,-0.5,1.5,-1.5,6,-6,-12,12];%[0.6,-0.6,2,-2,6,-6,12,-12];%,4,-4,8,-8,16,-16];
                anguloTargets = [0 45 -45 60 -60 90 -90 -135 135 -180 180];%
                vels = [800];
    ...
    pares = zeros(2,2);
    posiblesAngulosI = unique(results(:,4)); 
    posiblesAngulosF = unique(results(:,1));
    tiempoMin = 0.2;
    tiempoMax = 1.5;
    bins = 1:10;
    anguloTargets = [0 45 -45 90 -90 -135 135 -180 180];
    sesgoCat = [];
    pesoRecompensa = [0.5,0.5];
    ensayo = length(results) + 1;
end
    % Otras variables que en algun momento se utilizan... o no... ¿?
    previosCorrectos =  [-1 1];
    
    %inicializacion de variables necesarias
    anguloInicial = 0; anguloNuevo = 0; askNext = 0; nextAng = 0; rewardtime = 0; catPointer = 1; paresDisponibles = pares; %lista de pares disponibles
        ...
        case -1
            ...
            if newStims; % Si el ensayo anterior se completo, se seleccionan estimulos; si el ensayo se aborto se repiten los mismos. 
                ...
                elseif experimento == 4
                    for i = 1:10
                        maximos(i) = max(estructuras{i}.reversal);
                    end
                    maximos(maximos == 0) = 1/length(maximos);
                    total = sum(maximos);
                    wBin = 1-(maximos./total);
                    bin = randsample(1:10,1,true,wBin);
                    vel = round(1000*calcularTiempo(bin,length(bins),velMin,velMax,0.1));
                    anguloInicial = randsample(posiblesAngulosI,1);
                    anguloNuevo = getX(estructuras,bin);
                    
                    signo2 = randsample([-1,1], 1, 'true', sesgo);
                    anguloNuevo = abs(anguloNuevo)*signo2;
                    category = categorization2(anguloInicial,anguloNuevo);
                    tiempo = calcTiempo(anguloNuevo,vel);
                    
                elseif experimento == 6
                    ...
                    tiempoS = calcularTiempo(bin,length(bins),tiempoMin,tiempoMax);
                    anguloInicial = randsample(posiblesAngulosI,1);
                    anguloNuevo = getX(estructuras,bin);
                    signo2 = randsample([-1,1], 1, 'true', sesgo);
                    anguloNuevo = abs(anguloNuevo)*signo2;
                    category = categorization2(anguloInicial,anguloNuevo);
                    vel = calculoVelocidad(anguloNuevo,tiempoS);
...
%             end
    ...
    %%                       
                ...
                for i = 1:length(bins)
                    ...
                    hold on
                    bin2 = bins(i);
                    indx = results(:,11) == bin2;
                    results2 = results(indx,:);

                    indxPos = sign(results2(:,1)) == 1;
                    indxNeg = sign(results2(:,1)) == -1;



                    indx2 = results2(:,3) == 1;
                    ind = find(indxPos.*indx2);
                    plot(ind,results2(ind,1), 'ob', 'MarkerFaceColor', 'b')
                    correctasP = sum(indxPos.*indx2);

                    indx2 = results2(:,3) == 0;
                    ind = find(indxPos.*indx2);
                    plot(ind,results2(ind,1), '*b')
                    incorrectasP = sum(indxPos.*indx2);

                    indx2 = results2(:,3) == 1;
                    ind = find(indxNeg.*indx2);
                    plot(ind,abs(results2(ind,1)), 'oy', 'MarkerFaceColor', 'y')
                    correctasN = sum(indxNeg.*indx2);

                    indx2 = results2(:,3) == 0;
                    ind = find(indxNeg.*indx2);
                    plot(ind,abs(results2(ind,1)), '*y')
                    set(gca, 'Color', 'k')
                    xlabel('Ensayo')
                    incorrectasN = sum(indxNeg.*indx2);

                    pos = correctasP/(correctasP+incorrectasP)
                    neg = correctasN/(correctasN+incorrectasN)
                    %title(['bin: ',num2str(bin), ])
                    titulo = PAL_AMUD_analyzeUD(estructuras{bin2},'reversals',5);
                    xlabel(['Reversiones:',num2str(max(estructuras{i}.reversal))])
                    plot(1:100,titulo,'-r')
                    title(['bin:',num2str(i),' thr = ', num2str(titulo)])
                    if length(results2) > 100
                        axis([length(results2)-100,length(results2),0,16])
                    else                    
                        axis([0,100,0,16])
                    end
                end
                ...
                if length(results) > 50
                    results2 = results(length(results)-rango:end,:);
                else
                    results2 = results;
                end
                ...
                plot(ind,abs(results2(ind,1)), '*y')
                set(gca, 'Color', 'k')
                xlabel(['Neg:',num2str(sesgo(1)),'Pos:',num2str(sesgo(2))])
                incorrectasN = sum(indxNeg.*indx2);

                pos = correctasP/(correctasP+incorrectasP)
                neg = correctasN/(correctasN+incorrectasN)
                title(['pos: ',num2str(pos), 'neg: ', num2str(neg)])
                ...
                plot(results2(:,11), '.k');
                axis([0,55,0,11])
                title(['num de ensayos: ', num2str(ensayo)])
                xlabel(['bin: ', num2str(bin)])
                ylabel('bin');
                %tightfig
                %set(gcf, 'position', [75 1 1846 994]);
            ...
            results(ensayo,11) = bin;
            %updates estructuras with the answer, 0 incorrect, 1 correct
            estructuras{bin} = PAL_AMUD_updateUD(estructuras{bin}, respuesta == sign(anguloNuevo));
data.anguloNuevo = anguloNuevo;
                data.respuesta = respuesta;
                data.acierto = respuesta == sign(anguloNuevo);
                data.anguloInicial = anguloInicial;
                data.angTarget = angTarget;
                data.voltStamp = round(voltStamp/100);
                data.timeStamp = results(ensayo,7)/1000;
                data.time = tiempo;
                data.velocidad = vel;
                data.category = results(ensayo,10);
                data.x = x;
                data.y = y;
                data.bin = bin;
                sendAllToBlackRock(dioBlackRock,data); %sends data to BLACKROCK
            save(archivoExistente,'results','programa', 'experimento', 'estructuras')
